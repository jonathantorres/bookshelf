...
class Pinger:
    ...
    def receive_ping(self, my_socket, dest_addr):
        ...
        icmp_packet = socket_data[20:]

        ip_src = ip_packet_ba[12:16]
        ip_ttl = ip_packet_ba[8]
        icmp_length = len(icmp_packet)
        icmp_checksum = icmp_packet[2:4]
        icmp_id = icmp_packet[4:6]
        icmp_seq = icmp_packet[6:8]
        icmp_date = icmp_packet[-8:]
        ...
        icmp_packet[2:4] = b"\x00\x00"

        if sys.platform == "darwin":
            reply_checksum = icmp_checksum & 0xffff
        else:
            reply_checksum = icmp_checksum
        local_checksum = self.checksum(bytes(icmp_packet))
    ...
    def send_ping(self, my_socket, dest_addr):
        ...
        header = struct.pack("testheader", ICMP_ECHO_REQUEST, 0, packet_checksum, self.icmp_id, self.icmp_seq)
