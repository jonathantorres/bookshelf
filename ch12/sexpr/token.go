...
type TokDecoder struct {
	scan  scanner.Scanner
	err   error
	depth int
}

func NewTokDecoder(r io.Reader) *TokDecoder {
	var scan scanner.Scanner
	scan.Init(r)
	dec := TokDecoder{scan: scan}
	scan.Error = dec.setError
	return &dec
}

func (d *TokDecoder) setError(scan *scanner.Scanner, msg string) {
	d.err = fmt.Errorf("scanning: %s", msg)
}

func (d *TokDecoder) Token() (Token, error) {
	t := d.scan.Scan()
	if d.err != nil {
		return nil, d.err
	}
