...
int DArray_push_sorted(DArray *array, void *el, DArray_compare cmp)
{
    DArray_push(array, el);
    return DArray_qsort(array, cmp);
}

void DArray_print(DArray *array, char element_type)
{
    ...
    for (int i = 0; i < array->end; i++) {
        if (array->contents[i] != NULL) {
            switch (element_type) {
                case 'i':
                    printf("%d,", *((int *)array->contents[i]));
                    break;

                case 'f':
                    printf("%f,", *((float *)array->contents[i]));
                    break;
                ...
                case 'c':
                    printf("%c,", *((char *)array->contents[i]));
                    break;

                default:
                    printf("%p,", array->contents[i]);
            }
        }
    }
