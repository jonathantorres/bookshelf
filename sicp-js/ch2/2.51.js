...
x_painter(f);
display("below -----------------------");
below(x_painter, x_painter)(f);

display("stack -----------------------");
stack(x_painter, x_painter)(f);

function stack(painter1, painter2) {
    const rot1 = rotate90(painter1);
    const rot2 = rotate90(painter2);

    return rotate270(beside(rot1, rot2));
}

function below(painter1, painter2) {
    const split_point = make_vect(0, 0.5);
    const paint_up = transform_painter(
        painter1,
        split_point,
        make_vect(1, 0.5),
        make_vect(0, 1)
    );
    const paint_down = transform_painter(
        painter2,
        make_vect(0, 0),
        make_vect(1, 0),
        split_point
