...
install_sparse_term_list_package();
install_dense_term_list_package();
...
// TODO: Fix representation for these new added types of term lists
// display_list(add(p1, p2));
...
function install_sparse_term_list_package() {
    ...
    put("first_term", "sparse", first_term);
    put("rest_terms", "sparse", term_list => tag(rest_terms(term_list)));
    put("adjoin_term", "sparse", (term, term_list) => tag(adjoin_term(term, term_list)));
    put("make", "sparse", () => tag(list()));
}

function install_dense_term_list_package() {
    // representation of terms and term lists
    function adjoin_term(term, term_list) {
        return is_equal_to_zero(coeff(term))
               ? term_list
    ...
    put("first_term", "dense", first_term);
    put("rest_terms", "dense", term_list => tag(rest_terms(term_list)));
    put("adjoin_term", "dense", (term, term_list) => tag(adjoin_term(term, term_list)));
}
...
function adjoin_term(term, term_list) { // FINISH THIS
    return get("adjoin_term", type_tag(term_list), term, term_list);
}

function first_term(term_list) {
    return apply_generic("first_term", list(term_list));
}

function rest_terms(term_list) {
    return apply_generic("rest_terms", list(term_list));
}
