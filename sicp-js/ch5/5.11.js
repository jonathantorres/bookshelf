import {
    display, display_list, pair, head, tail, is_null, list, append, map, is_undefined,
    accumulate, list_ref, set_head, length, error, is_string, set_tail,
    apply_in_underlying_javascript, is_boolean, for_each, equal, is_pair, filter
} from 'sicp';

const m = make_machine(
    list("continue", "n", "val"),
    list(
        list("*", (a, b) => a * b),
        list("-", (a, b) => a - b),
...
set_register_contents(m, "n", 10);
start(m);
display(get_register_contents(m, "val"));
display_list(get_data_path_data(m));

function get_data_path_data(machine) {
    return machine("get_data_path");
}

function parse_data_path_data(controller_text) {
    let instructions = [];
    let registers = [];
    let saved = [];
    let assigned = [];
    ...
    for_each(l => {
        if (is_pair(l) && !instructions.includes(head(l))) {
            instructions.push(head(l));
        }
    }, controller_text);

    for_each(l => {
        if (is_pair(l) && head(l) === "go_to" && head(head(tail(l))) === "reg") {
            const reg = head(tail(l));
            if (!registers.includes(head(tail(reg)))) {
                registers.push(head(tail(reg)));
            }
        }
    }, controller_text);

    for_each(l => {
        if (is_pair(l) && (head(l) === "save" || head(l) === "restore")) {
            const reg = head(tail(l));
            if (!saved.includes(reg)) {
                saved.push(reg);
            }
        }
    }, controller_text);
    ...
    for_each(l => {
        if (is_pair(l) && head(l) === "assign") {
            assigned.push(head(tail(tail(l))));
        }
    }, controller_text);

    return list(
        list('instructions', instructions.sort()),
        list('registers', registers.sort()),
        list('saved_registers', saved.sort()),
        list('assigned_registers', assigned.sort()),
    );
}

function process_text(controller_text, predicate, proc) {
    return map(proc, filter(predicate, filter(pair_p, controller_text)));
}
...
function make_new_machine() {
    ...
    function dispatch(message) {
        ...
        return message === "start"
               ...
               : message === "install_data_path_data"
               ? info => { data_path = info; }
               : message === "get_data_path"
               ? data_path
               : message === "stack"
               ? stack
               : message === "operations"
...
function make_machine(register_names, ops, controller) {
    ...
    machine("install_data_path_data")
           (parse_data_path_data(controller));
    return machine;
}
