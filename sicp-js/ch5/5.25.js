...
const ops = list(
    ...
    list("assignment_value_expression", assignment_value_expression),
    list("assign_symbol_value" , assign_symbol_value),
    list("is_declaration"      , is_declaration),
    list("declaration_symbol"  , declaration_symbol),
    list("declaration_value_expression", declaration_value_expression),
    list("assign_symbol_value" , assign_symbol_value),
    ...
    list("is_while_loop"      , is_while_loop),
...
const evaluator = make_machine(
    ...
    list(
        "read_program",
            ...
            assign("env", list(op("extend_environment"), reg("val"), reg("comp"), reg("env"))),
        ...
        "eval_dispatch",
            ...
            test(list(op("is_while_loop"), reg("comp"))),
            branch(label("ev_while_loop")),
        ...
        "ev_lambda",
            ...
            assign("val", list(op("make_function"), reg("unev"), reg("comp"), reg("env"))),
            go_to(reg("continue")),
        "ev_conditional",
        ...
        "ev_operator_combination",
            assign("comp", list(op("operator_combination_to_application"), reg("comp"), reg("env"))),
        "ev_application",
            save("continue"),
            save("env"),
        ...
        "ev_while_loop",
            ...
            assign("unev", list(op("arg_expressions"), reg("comp"))),
            save("unev"),
            assign("comp", list(op("eval_while"), reg("comp"), reg("env"))),
            restore("env"),
            assign("continue", label("apply_dispatch")),
            go_to(label("eval_dispatch")),
        "apply_dispatch",
        ...
        "ev_assignment_install",
            ...
            restore("env"),
            restore("unev"),
            perform(list(op("assign_symbol_value"), reg("unev"), reg("val"), reg("env"))),
            go_to(reg("continue")),
        ...
        "ev_declaration",
            ...
            save("unev"),
            assign("comp", list(op("declaration_value_expression"), reg("comp"))),
            save("env"),
            save("continue"),
        ...
        "unknown_function_type",
            ...
            go_to(label("signal_error")),
        "signal_error",
            perform(list(op("user_print"), constant("EC-evaluator error:"), reg("val"))),
            go_to(label("done")),
        "done"
    )
);
...
function make_execution_function(inst, labels, machine, pc, flag, stack, ops) {
    ...
    return inst_type === "assign"
           ...
           : inst_type === "push_marker_to_stack"
           ? make_push_marker_to_stack_ef(machine, stack, pc)
           : inst_type === "revert_stack_to_marker"
...
function eval_while(component, env) {
    ...
    const body = while_loop_block(component);

    function loop() {
        if (evaluate(pred, env)) {
            const comp = evaluate(body, env);

            if (is_return_statement(comp) && is_literal(return_expression(comp))) {
                const lit = literal_value(return_expression(comp));
                if (lit === "break") {
                    return;
                }
                if (lit === "continue") {
...
function while_loop_predicate(component) {
    return head(tail(component));
}

function while_loop_block(component) {
