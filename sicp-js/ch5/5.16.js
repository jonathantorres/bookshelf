...
set_register_contents(exp_machine, "n", 10);
start(exp_machine);
display(get_register_contents(exp_machine, "product"));

function make_new_machine(enable_tracing) {
...
function make_machine(register_names, ops, controller, enable_tracing) {
    const machine = make_new_machine(enable_tracing);
