...
const m = make_machine(
    ...
    list(
        "start",
            go_to(label("here")),
        // uncomment this label to trigger the error
        // "here",
            assign("a", constant(3)),
            go_to(label("there")),
        "here",
...
function label_exists(labels, label) {
    return assoc(label, labels);
}
                 ...
                 (insts, labels) => {
                   const next_element = head(controller);
                     if (is_string(next_element)) {
                         if (!is_undefined(label_exists(labels, next_element))) {
                             return error(next_element, "Duplicated label -- extract_labels");
                         }
                        return receive(insts, pair(make_label_entry(next_element, insts), labels));
                     }
                     return receive(pair(make_inst(next_element), insts), labels);
                 });
}
