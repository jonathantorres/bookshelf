import {
    ...
    tail, set_head, set_tail, error, member, append, display_list, reverse
} from 'sicp';
...
ripple_carry_adder(a_wires, b_wires, c_in, sum_wires, c_out);

display(get_signal(c_out));

function ripple_carry_adder(a, b, c_in, s, c_out) {
    function ripple(a, b, s, c) {
        if (is_null(tail(a))) {
            full_adder(head(a), head(b), c_in, head(s), c);
            return "ok";
        }
        const w = make_wire();
        full_adder(head(a), head(b), w, head(s), c);
        ripple(tail(a), tail(b), tail(s), w);
    }
    ripple(reverse(a), reverse(b), reverse(s), c_out);
    return "ok";
}
