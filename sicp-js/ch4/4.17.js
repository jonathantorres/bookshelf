import {
    ...
    apply_in_underlying_javascript, is_boolean, reverse, for_each
} from 'sicp';
...
evaluate(tree, the_global_environment);
...
// we simply have to evaluate every function declaration in a first pass
// on the sequence of statements before we evaluate anything else
function eval_sequence(stmts, env) {
    for_each((stmt) => {
        if (is_function_declaration(stmt)) {
            evaluate(stmt, env);
        }
    }, stmts);

    function eval_seq(stmts, env) {
        if (is_empty_sequence(stmts)) {
            ...
            if (is_return_value(first_stmt_value)) {
                ...
                return eval_seq(rest_statements(stmts), env);
            }
        }
    }

    return eval_seq(stmts, env);
}
