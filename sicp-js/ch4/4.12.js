...
const prog = "{ y = 0; display(y); let y = 0; }";
...
// also check for unassigned values
function lookup_symbol_value(symbol, env) {
    ...
    if (value_is_unassigned(val)) {
        return error(symbol, "value is unassigned");
    }
...
function assign_symbol_value(symbol, val, env) {
    function env_loop(env) {
        function scan(symbols, vals) {
            return is_null(symbols)
                   ? env_loop(enclosing_environment(env))
                   : value_is_unassigned(head(vals))
                   ? error(symbol, "value is unassigned")
                   : symbol === head(symbols)
                   ? set_head(vals, val)
                   : scan(tail(symbols), tail(vals));
        }
...
function value_is_unassigned(value) {
