...
const prog = "{ let x = 5; x = 6; display(x); }";
...
// re-written representation of frames
function make_frame(symbols, values) {
    function loop(symbs, vals, frame) {
        if (is_null(symbs)) {
            return null;
        }
        return pair(pair(head(symbs), head(vals)), loop(tail(symbs), tail(vals)));
    }
    return loop(symbols, values, null);
}
...
function frame_symbols(frame) {
    ...
    for_each((p) => {
        symbols = append(symbols, list(head(p)));
    }, frame);
    return symbols;
}
