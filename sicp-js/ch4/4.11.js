...
// use a pair to distinguish between constants
    ...
    function env_loop(env) {
        function scan(symbols, vals) {
            return is_null(symbols)
                   ...
                   : is_pair(head(symbols)) && symbol === tail(head(symbols))
                   ? set_head(vals, val)
                   : symbol === head(symbols)
                   ? set_head(vals, val)
        ...
        if (env === the_empty_environment) {
            error(symbol, "unbound name -- assignment to constant");
        } else {
            const frame = first_frame(env);
            display_list(frame, "frame");
    ...
    function env_loop(env) {
        function scan(symbols, vals) {
            return is_null(symbols)
                   ? env_loop(enclosing_environment(env))
                   : is_pair(symbol)
                   ? error(tail(symbol), "cannot assign to constant")
                   : symbol === head(symbols)
                   ? set_head(vals, val)
                   : scan(tail(symbols), tail(vals));
        }
...
// selectors for contants and variables
