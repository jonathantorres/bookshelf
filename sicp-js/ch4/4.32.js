import {
    ...
    apply_in_underlying_javascript, is_boolean, reverse, set_tail
} from 'sicp';

// our custom (lazy) list implementation
function my_pair(x, y) {
    return m => m(x, y);
}
...
function my_list(...args) {
    function make_list(args) {
        if (args.length === 0) {
            return null;
        }

        return my_pair(args.shift(), make_list(args));
    }

    return make_list(args);
}

const primitive_functions = list(list("head", my_head),
                                 list("tail", my_tail),
                                 list("pair", my_pair),
                                 list("list", my_list),
...
const prog = `
    ...
    function display_pair(p) {
        display('[' + head(p) + ', ' + stringify(tail(p)) + ']');
    }
...
const locals = scan_out_declarations(tree);
const unassigneds = list_of_unassigned(locals);
const program_env = extend_environment(locals, unassigneds, the_global_environment);
const output = actual_value(tree, program_env);
