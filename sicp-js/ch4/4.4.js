...
const prog = "{ let s = 0; if (true && true) { display(s); } }";
...
function evaluate(component, env) {
    return is_literal(component)
           ...
           : is_logical_or(component)
           ? eval_or(component, env)
           : is_logical_and(component)
...
function eval_or(component, env) {
    const exp1 = evaluate(head(tail(tail(component))), env);
    const exp2 = evaluate(head(tail(tail(tail(component)))), env);
    return exp1 ? true : exp2;
}
...
function is_logical_or(component) {
    return is_tagged_list(component, "logical_composition") && head(tail(component)) === "||";
}

function is_logical_and(component) {
    return is_tagged_list(component, "logical_composition") && head(tail(component)) === "&&";
}
