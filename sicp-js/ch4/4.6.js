import {
    ...
    apply_in_underlying_javascript, is_boolean, reverse, tokenize
} from 'sicp';
...
const prog = "{ let* x = 3; let* y = x+2; let* z = x+y+5; display(x*z); }";
...
// parse rule: << let* name = expression; >>
// list("let_star_declaration", << name >>, << expression >> )
function is_let_star_declaration(component) {
    return is_tagged_list(component, "let_star_declaration");
}

// let* selectors
function let_star_name(component) {
    return symbol_of_name(component);
}

function let_star_expression(component) {
    return head(tail(tail(component)));
}

// nested let evaluation
function let_star_to_nested_let(component) {
    return make_block(
        list("sequence", list(
            make_variable_declaration(
                let_star_name(component),
                evaluate(let_star_expression(component))
            ),
        ))
    );
}
