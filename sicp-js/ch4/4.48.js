import {
    ...
    apply_in_underlying_javascript, is_boolean, reverse, set_tail, math_PI, math_random
} from 'sicp';
...
rambeval(parse(prog), the_global_environment,
    // rambeval success
...
function rambeval(component, env, succeed, fail) {
...
function shuffle(lst) {
    function remove(x, lst) {
        ...
        pair(head(lst), remove(x, tail(lst)));
    }
    function get(n, lst) {
        if (n === 0) {
            return head(lst);
        }
        return get(n-1, tail(lst));
    }
    if (is_null(lst)) {
        return null;
    }
    const x = get(math_random()*length(lst), lst);

    return pair(x, shuffle(remove(x, lst)));
}
